...
2. Elegí el motor de conversión: `MarkItDown` o `MinerU`.
3. Seleccioná o arrastrá un archivo soportado.
4. Presioná `Convertir`.
5. Revisá el Markdown generado en la vista previa.
6. Descargá el `.md` o copiá el contenido al portapapeles.

Durante la conversión, la pantalla muestra el paso actual del proceso. Al terminar informa el motor usado, el tiempo demandado, la ruta guardada y el tamaño del Markdown generado.
...
- El mensaje de estado confirma la conversión e informa cuánto tardó.
...
Si elegís MinerU y la dependencia opcional no está instalada, la interfaz muestra un error accionable con dos caminos: instalar MinerU con `uv sync --extra dev --extra mineru` o volver a convertir con MarkItDown.
...
El selector de archivos del navegador usa esas extensiones como filtro inicial para que sea más fácil elegir documentos válidos. La validación definitiva se hace siempre en el backend.
...
La interfaz usa el endpoint de conversión:
...
- MinerU seleccionado sin tener instalada la dependencia opcional.

También expone un endpoint simple de salud para chequeos externos:
...
- `mi_markitdown/web.py`: crea la aplicación FastAPI y registra rutas web, API y salud.
...
El `pyproject.toml` configura `pytest` para ejecutar la suite con salida resumida. La suite cubre validaciones de conversión, motores, rutas web principales, favicon, salud y el contrato básico de `POST /api/convert`.
...
El código fuente propio de este repositorio se distribuye bajo licencia MIT.

Los materiales didácticos propios —apuntes, consignas, presentaciones, guías y textos— se distribuyen bajo licencia Creative Commons Atribución-NoComercial-CompartirIgual 4.0 Internacional (CC BY-NC-SA 4.0), salvo indicación en contrario.

Los materiales de terceros conservan sus respectivas licencias y derechos de autor.
